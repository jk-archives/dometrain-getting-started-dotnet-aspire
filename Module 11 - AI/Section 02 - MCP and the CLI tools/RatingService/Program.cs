...
app.MapPost("/ratings/batch", async ([FromBody] string[] podcastNames, RatingStore ratingStore) =>
{
    if (podcastNames is null || podcastNames.Length == 0)
        return Results.BadRequest("Please provide at least one podcast name.");

    var ratings = await ratingStore.GetAverageRatings(podcastNames);

    Tracer.CurrentSpan.SetAttribute("Count", podcastNames.Length);
