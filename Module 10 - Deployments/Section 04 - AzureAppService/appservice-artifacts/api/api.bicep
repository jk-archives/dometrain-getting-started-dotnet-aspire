  ...
  properties: {
    ...
    siteConfig: {
      ...
      appSettings: [
        ...
        {
          name: 'RATINGSERVICE_HTTP'
          value: 'http://${take('${toLower('ratingservice')}-${uniqueString(resourceGroup().id)}', 60)}.azurewebsites.net'
        }
        {
          name: 'services__ratingservice__http__0'
          value: 'http://${take('${toLower('ratingservice')}-${uniqueString(resourceGroup().id)}', 60)}.azurewebsites.net'
        }
        {
          name: 'RATINGSERVICE_HTTPS'
          value: 'https://${take('${toLower('ratingservice')}-${uniqueString(resourceGroup().id)}', 60)}.azurewebsites.net'
        }
        {
          name: 'services__ratingservice__https__0'
          value: 'https://${take('${toLower('ratingservice')}-${uniqueString(resourceGroup().id)}', 60)}.azurewebsites.net'
        }
        {
...
resource slotConfigNames 'Microsoft.Web/sites/config@2025-03-01' = {
  ...
  properties: {
    appSettingNames: [
      'RATINGSERVICE_HTTP'
      'services__ratingservice__http__0'
      'RATINGSERVICE_HTTPS'
      'services__ratingservice__https__0'
      'OTEL_SERVICE_NAME'
    ]
  }
  parent: webapp
