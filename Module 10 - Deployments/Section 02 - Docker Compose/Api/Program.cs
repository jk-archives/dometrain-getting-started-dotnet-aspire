...
builder.Services.AddHttpClient<RatingServiceHttpClient>(x => x.BaseAddress = new Uri("http://ratingservice"));
...
// In production, this would typically be a CI/CD pipeline step.
using (var scope = app.Services.CreateScope())
{
    var db = scope.ServiceProvider.GetRequiredService<PodcastDbContext>();
    await db.Database.MigrateAsync();

    if (!await db.Podcasts.AnyAsync())
    {
        db.Podcasts.AddRange(
            new Podcast(Guid.NewGuid(), "Unhandled Exception podcast"),
            new Podcast(Guid.NewGuid(), "Keep Coding Podcast"),
            new Podcast(Guid.NewGuid(), "Developer Weekly Podcast"),
            new Podcast(Guid.NewGuid(), "The Stack Overflow Podcast"),
            new Podcast(Guid.NewGuid(), "The Hanselminutes Podcast"),
            new Podcast(Guid.NewGuid(), "The .NET Rocks Podcast"),
            new Podcast(Guid.NewGuid(), "The Azure Podcast"),
            new Podcast(Guid.NewGuid(), "The AWS Podcast"),
            new Podcast(Guid.NewGuid(), "The Rabbit Hole Podcast"),
            new Podcast(Guid.NewGuid(), "The .NET Core Podcast")
        );
